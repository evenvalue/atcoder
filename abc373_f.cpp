...
inline void solution() {
  ...
  vector<max_heap<int>> delta_next(W + 1);
  for (int i = 0; i < n; i++) {
    const int w = read<int>();
    delta_next[w].push(read<int>() - 1);
  }
  ...
  for (int w = 1; w <= W; w++) {
    if (delta_next[w].empty()) continue;

    val = {0};
    int lim = W / w;
    while (lim--) {
      const int delta = delta_next[w].top();
      val.push_back(val.back() + max(0, delta));
      delta_next[w].pop();
      delta_next[w].push(delta - 2);
    }

    for (int i = W; i >= 0; i--) {
      for (int k = 1; k < val.size() and k * w <= i; k++) {
        dp[i] = max(dp[i], dp[i - k * w] + val[k]);
      }
    }
  }
