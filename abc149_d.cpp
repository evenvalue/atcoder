...
void solution() {
  ...
  const int r = readInt(), s = readInt(), p = readInt();
  const string bot = readString();

  auto play = [](const char bot_hand) {
    if (bot_hand == 'r') {
      return 'p';
    } else if (bot_hand == 'p') {
      return 's';
  ...
  auto last = [&](const int i) {
    return (i < k ? '-' : plays[i - k]);
  };

  int ans = 0;
  for (int i = 0; i < n; i++) {
    if (const char hand = play(bot[i]); last(i) != hand) {
      ans += points(hand);
      plays[i] = hand;
    }
  }
